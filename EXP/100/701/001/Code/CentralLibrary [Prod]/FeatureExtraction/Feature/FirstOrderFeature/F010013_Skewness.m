classdef F010013_Skewness < FirstOrderFeature
    %Image
    %
    % Based on Aerts et al. 2014, DOI: 10.1038/ncomms5006
    % Equation is on page 13 of supplementary materials
    ...
    properties (Constant = true, GetAccess = public)
        sFeatureName = "F010013"
        sFeatureDisplayName = "Skewness"
    end    
    ...
    methods (Access = protected)
        function dValue = ExtractFeature(obj,oImage, oFeatureExtractionParameters)
            ...
            %
            % DESCRIPTION:
            %  Extracts the skewness feature value.
            %
            % INPUT ARGUMENTS:
            ...
            dNumerator = 1/numel(dMaskedImage)*sum((dMaskedImage-dMean).^3);
            dDenominator = sqrt(1/numel(dMaskedImage).*sum((dMaskedImage-dMean).^2)).^3;
