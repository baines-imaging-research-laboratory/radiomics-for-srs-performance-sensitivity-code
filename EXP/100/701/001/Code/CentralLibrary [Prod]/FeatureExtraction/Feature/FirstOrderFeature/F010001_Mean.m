classdef F010001_Mean < FirstOrderFeature
    %Image
    %
    ...
    methods (Access = protected)
        function dValue = ExtractFeature(obj, oImageVolumeHandler, oFeatureExtractionParameters)
            ...
            %
            % DESCRIPTION:
            %  Extracts the mean feature value.
            %
            % INPUT ARGUMENTS:
            ...
            % Get mean
            [m3xImageData, m3bMask] = oImageVolumeHandler.GetCurrentRegionOfInterestImageDataAndMask(oFeatureExtractionParameters);
            
            dValue = sum(m3xImageData(m3bMask)) / sum(m3bMask(:)); % sum(m3bMask(:)) = numel(m3xImageData(m3bMask))
        end
