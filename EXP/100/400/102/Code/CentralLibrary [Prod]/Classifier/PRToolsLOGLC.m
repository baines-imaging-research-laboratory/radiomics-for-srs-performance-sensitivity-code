classdef PRToolsLOGLC < PRToolsClassifier
    %PRToolsLOGLC
    %
    % PRToolsLOGLC Computation of the linear classifier for the dataset by 
    ...
    properties (SetAccess = immutable, GetAccess = public)
        sName = "PRTools Logistic Linear Classifier";
        hClassifier = [];
    ...
    % *********************************************************************
    methods
        function obj = PRToolsLOGLC(chClassifierHyperParametersFileName,oHyperParameterOptimizer)      
            %obj = PRToolsLOGLC(chClassifierHyperParametersFileName)
            %
            % SYNTAX:
            %  obj = PRToolsLOGLC(chClassifierHyperParametersFileName)
            %
            % DESCRIPTION:
            %  Constructor for PRToolsLOGLC, it assigns the mapping and even though it has a path for
            ...
            obj.hClassifier = @loglc; % This is a PRTools "mapping"
        end
    end
    ...
    methods (Access = protected)
        function c1xHyperParams = GetImplementationSpecificParameters(obj) 
            ...
            % There are no hyperparameters for LOGLC
            c1xHyperParams = {};
        end
    end
end