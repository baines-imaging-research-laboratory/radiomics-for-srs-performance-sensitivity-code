classdef F010016_10Percentile < FirstOrderFeature
    %Image
    %
    ...
    properties (Constant = true, GetAccess = public)
        sFeatureName = "F010016"
        sFeatureDisplayName = "10th Percentile"
    end    
    ...
    methods (Access = public)
        ...
        function obj = F010016_10Percentile()
        end
    end
    methods (Access = protected)
        function dValue = ExtractFeature(obj,oImage, oFeatureExtractionParameters)
            ...
            %
            % DESCRIPTION:
            %  Extracts the 10th percentile feature value.
            %
            % INPUT ARGUMENTS:
            ...
            % Set up the interpolation function handle
            fhPctlCustom = @(v,p) interp1(linspace(0,1,length(v))', sort(v), p*0.01, 'linear');
            
            % Calculate the percentile
            dValue = fhPctlCustom(double(m3xImageData(m3bMask)),10);
